...
namespace RecipeBook.ServiceLibrary.Repositories
{
    ...
    {
        Task<int> InsertAsync(SqlConnection connection, DbTransaction transaction, IEnumerable<IngredientEntity> entities);
...

    {
        ...
        public async Task<int> InsertAsync(SqlConnection connection, DbTransaction transaction, IEnumerable<IngredientEntity> entities)
        {
            if (entities is null)
            {
                return 0;
